...
case class Signature(paramsSig: List[ParamSig], resSig: TypeName) {
   ...
   *  collapses to `eq` once that chain unwinds (`Name.equals` is `this eq that`,
   *  Names.scala), so we hand-write a typed-arm match that emits `if_acmpeq` for
   *  `TypeName/TypeName`, primitive `if_icmpeq` for `Int/Int`, and a single
   *  `eq tpnme.Uninstantiated` check for the cross arms (an `Int` box can never be
   *  `eq` a `TypeName`, so the `nameX == Uninstantiated` disjuncts collapse to the
   *  one surviving check). Semantically identical to the original `==` disjunction.
   */
   ...
   *  `TypeName` `tpnme.Uninstantiated`, so they are skipped without any dispatch.
   */
