...
object Inlines:
  ...
  def inlineCall(tree: Tree)(using Context): Tree = ctx.profiler.onInlineCall(tree.symbol):
     ...
     *  Profiling shows this TreeTypeMap traversal is one of the dominant
     *  per-inline-call costs (~16,986 inlineCall × full tree walk), but in
     *  CC-disabled compilations the stripper is essentially a no-op:
     *  CleanupRetains only mutates a type when it sees an `AnnotatedType`
     *  with a `RetainingAnnotation`, which is generated by capture checking.
     *  So skip the traversal entirely when CC is not enabled.
     */
    val tree0 =
