...
class LocalOpt(backendUtils: BackendUtils, callGraph: CallGraph, inliner: Inliner,
  ...
  private def methodOptimizations(method: MethodNode, ownerClassName: InternalName): Boolean = {
    ...
    var lineNumberCleanupNeeded = BackendUtils.isDceDone(method)
    ...
    def removalRound(
      ...
      val runNullnessAgain = boxUnboxChanged || pushPopNullCheckAdded || callInlinedByStaleStores
    ...
    val lineNumbersRemoved =
      if (lineNumberCleanupNeeded) removeEmptyLineNumbers(method)
      else
    ...
    Limits.sizeOKForNullness(method) && {
      ...
      while (it.hasNext) it.next() match {
        ...
        case mi: MethodInsnNode =>
          ...
          // Explicitly check for `x.equals(null)` when x is known nonnull
...
object LocalOptImpls {
   ...
   */
  def minimalRemoveUnreachableCode(method: MethodNode, ownerClassName: InternalName, callGraph: CallGraph, backendUtils: BackendUtils): Boolean = {
    ...
    def removalRound(): Boolean = {
      val insnsRemoved = removeUnreachableCodeImpl(method, ownerClassName, callGraph, backendUtils)
      if (insnsRemoved) {
   ...
   *
   * When this method runs on a method with exception handlers, each `labelNode.getLabel` has a
   * status set whether the label is live or not. This can be queried using
   * `BackendUtils.isLabelReachable`.
   */
  def removeUnreachableCodeImpl(method: MethodNode, ownerClassName: InternalName, callGraph: CallGraph, backendUtils: BackendUtils): Boolean = {
    ...
    val recordLabelReachability = !method.tryCatchBlocks.isEmpty
    while (tcbIt.hasNext) {
      val tcb = tcbIt.next()
  ...
  def clearLabelReachableFlags(method: MethodNode): Unit = {
    ...
    while (iterator.hasNext) {
      iterator.next match {
        case label: LabelNode => BackendUtils.clearLabelReachable(label)
        case _ =>
      }
    }
  }
