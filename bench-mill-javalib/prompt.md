...
and earlier `iter-N/run-M/` artifacts such as `iter-N/research.md`, and propose **20
broad investigation areas** (algorithms, data structures, allocations, etc.).

# 2. Research

Spawn **20 parallel maximum-effort agents**, one per area. Require exhaustive
investigation before accepting their results: agents should spend time reading code,
walking call chains, checking profile-tree attribution, and cross-referencing prior
research before they commit to a proposal. Reject weak candidates during investigation.
...
- Read `git log -p main..accepted compiler/` (past wins) and `main..rejected compiler/`
  (past failures — avoid re-doing existing work, unless you have a new angle or the
  idea is promising enough the previous result may have been a fluke).
...
- If the first profile is borderline or unclear, the agent may profile another two times
  to try and get a more clear signal
...
Each implementation agent should finally write a `iter-n/run-m/report.md` file summarizing
the result of that run, and potentially 1-5 more potential optimizations that the orchestrator
should spawn a sub-agent to a validate and if verified spawn further implementation agents to
implement
